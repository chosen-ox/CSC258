...
mod test_range {
    ...
    fn test_chain() {
        ...
        let res = v.clone()
            .map(|x| x + 3)
            .chain(v.clone().map(|x| x + 6))
            .collect::<Vec<_>>();
        assert_eq!(res, vec![4, 5, 6, 7, 8, 9]);

        let res2 = v.clone()
            .flat_map(|x| [x + 5, x + 6])
            .chain(v.clone().flat_map(|x| [x + 8, x + 9]))
            .collect::<Vec<_>>();
        assert_eq!(res2, vec![6, 7, 7, 8, 8, 9, 9, 10, 10, 11, 11, 12]);

        let res3 = v.clone()
            .filter(|x| x > &2)
            .chain(v.clone().filter(|x| x > &2))
            .collect::<Vec<_>>();
        assert_eq!(res3, vec![3, 3]);
    }

    #[test]
    fn test_partition() {
        let v = 1..4;
        let (res, res2): (Vec<_>, Vec<_>) = v.clone()
            .filter_map(|x| {
                if x < 3
                { Some(x + 3) } else { None }
            })
        ...
        let (res3, res4): (Vec<_>, Vec<_>) = v.clone()
            .flat_map(|x| [x + 5, x + 6])
            .partition(|x| x > &7);
        assert_eq!(res3.iter().fold(0, |acc, x| acc + x), 25);
        assert_eq!(res4.iter().fold(0, |acc, x| acc + x), 20);

        let (res5, res6): (Vec<_>, Vec<_>) = v.clone()
            .map(|x| x + 3)
            .partition(|x| x > &4);
        assert_eq!(res5.iter().find(|x| x == &&5), Some(&5));
        assert_eq!(res6.iter().find(|x| x == &&4), Some(&4));
